...
int main(int argc, char** argv)
{
  ...
  image_transport::Subscriber image_raw_sub = it.subscribe(img_raw_topic, 100, imageCallback);
  image_transport::Subscriber image_mono_sub = it.subscribe(img_mono_topic, 100, imageCallback);
  image_transport::Subscriber image_color_sub = it.subscribe(img_color_topic, 100, imageCallback);
  image_transport::Subscriber image_rect_sub = it.subscribe(img_rect_topic, 100, imageCallback);
  image_transport::Subscriber image_rect_color_sub = it.subscribe(img_rect_color_topic, 100, imageCallback);
  image_transport::Subscriber<CameraInfo> camera_info_sub(nh, camera_info_topic, 100);

  image_transport::Publisher image_raw_pub = it.advertise(img_raw_topic, 1000);
  image_transport::Publisher image_mono_pub = it.advertise(img_mono_topic, 1000);
  image_transport::Publisher image_color_pub = it.advertise(img_color_topic, 1000);
  image_transport::Publisher image_rect_pub = it.advertise(img_rect_topic, 1000);
  image_transport::Publisher image_rect_color_pub = it.advertise(img_rect_color_topic, 1000);
  camera_info_pub = nh.advertise<CameraInfo>(camera_info_topic, 1000);
